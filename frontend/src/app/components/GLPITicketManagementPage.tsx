...
export default function GLPITicketManagementPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Coluna 1: Formulário de Abertura de Ticket (GLPI) */}
      <div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-6 h-fit space-y-5 lg:col-span-1">
        <div>
        ...
        <div className="bg-white/[0.04] p-1 rounded-lg border border-white/[0.08] flex text-[10px] font-semibold">
          <button
            type="button"
            ...
            <>
              ...
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-lg p-3 space-y-2">
                <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-1">
                    <p className="font-semibold text-white">Download Template</p>
                    <p className="text-[10px] text-slate-500 leading-relaxed">
                      Download the standard CSV template, fill the rows with your hosts, and upload below.
                    </p>
                  </div>
                </div>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-blue-400 hover:text-blue-300 font-bold transition-colors cursor-pointer bg-transparent border-0 p-0 text-left text-xs"
                >
              ...
              <div className="space-y-1.5">
                ...
                {!selectedFile ? (
                  <label className="flex flex-col items-center justify-center border border-dashed border-white/[0.12] hover:border-blue-500/50 rounded-lg p-5 cursor-pointer bg-white/[0.01] hover:bg-blue-500/[0.01] transition-all group">
                    <UploadCloud className="w-8 h-8 text-slate-500 group-hover:text-blue-400 transition-colors mb-2" />
                    <span className="font-semibold text-slate-300 group-hover:text-white text-center">Select CSV</span>
                    <span className="text-[9px] text-slate-500 mt-1">Only .csv files</span>
                    <input
                      type="file"
                      accept=".csv"
                      className="hidden"
                  ...
                  <div className="flex items-center justify-between border border-white/[0.08] bg-white/[0.02] rounded-lg p-3">
      ...
      {/* Colunas 2 e 3: Histórico de Tickets Abertos */}
      <div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-6 space-y-4 lg:col-span-2">
        <div>
          <h2 className="text-sm font-bold text-white tracking-wide flex items-center justify-between">
            <span>Ticket History</span>
            <Clock className="w-4 h-4 text-slate-500" />
          </h2>
          <p className="text-[11px] text-slate-500 mt-1">Track the triage and validation status of tickets opened by your team.</p>
        </div>

        <div className="overflow-x-auto border border-white/[0.05] rounded-xl text-xs">
          {loading ? (
                ...
                {tickets.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-white/[0.04] hover:bg-white/[0.01] transition-colors text-slate-300">
                      <td className="px-4 py-3.5 font-bold font-mono text-white text-[11px] select-all">{t.ticket_number}</td>
                      ...
                      <td className="px-4 py-3.5 uppercase font-semibold text-slate-400 flex items-center gap-1 mt-1">
                      ...
                      <td className="px-4 py-3.5 text-right text-slate-500 font-mono">{dateStr}</td>
                    </tr>
                  );
                })}
              </tbody>
